...
export function AdminCampaignFormPage() {
  ...
  const fetcher = useCallback(() => (id ? getCampaignAdmin(Number(id)) : Promise.resolve(null)), [id])
  ...
  const onSubmit = async (values: CampaignFormValues) => {
    ...
    try {
      ...
      navigate(ROUTES.adminCampaigns)
    } catch (err) {
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 grid gap-5">
        ...
        <FormField id="image" label="Campaign image">
          <Input id="image" type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => void onFileChange(e)} />
        ...
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" className="size-4 rounded border-input accent-primary" {...register('featured')} />
        ...
        <div className="flex gap-3">
          ...
          <Button type="button" variant="secondary" onClick={() => navigate(ROUTES.adminCampaigns)}>
            Cancel
          </Button>
        </div>
      </form>
